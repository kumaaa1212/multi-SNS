...
### バックエンド
バックエンド技術選定を行う際に選択肢として、**Node.jsとRuby(Ruby on Rails)** があった。しかし今回の技術選定の全体の軸として【円滑な開発】としていたため、フロント側でも使用しているTypeScriptを使用できる**Node.js**を選択した。TypeScriptによる静的型付けでの開発時からの保守性の高さ、Node.jsの特徴である　**大量の同時接続に強い**　などもSNSプロダクトでは大きなメリットとなると感じた。そして今回のプロダクトは**フォルダ数、データベース**ともにこれまで自身で開発していたものと比べ、複雑になってしまうことが想定できた。これまでFirebaseなどのNoSQLを使用した開発を行っていたが、今回はプロダクトの大きさやデータベースの複雑性を予想し、**RDBで設計を行うこと**にした。そしてこれまで使用経験が多い**Prisma**を使い、RDBを設計した。
