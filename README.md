# TOKOTOKO=J V2
**現在TOKOTOKO=Jのリプレスを行っています。詳細はフロントエンド・バックエンド各リポジトリに記載しています。（随時更新）**
<br />
<br />
【リプレイスに至った経緯】
- 実際に使っていただいたユーザーの声から、もっと質の高いプロダクトを作成したいと思った。
- フロントエンドだけではなく、バックエンドやデータベースなど多くの知識の選択が可能になった。（以前は選択肢が少なく、やりたいけどできないことが多かった。）
- 多くの技術的負債があった。
以上の3点が挙げられます。
<br />
<br />

**TOKOTOKO=Jは2023年7月-９月にこれまでの独学の集大成として作成しました。その当時の自分としてかなり完成度が高く、ユーザーの友人やサッカーファンからのポジティブなコメントなどもあり、納得いく形にできたと感じています。**
  <br />
**しかしその後多くのチーム開発などを通して、エンジニアとしての幅が広がりました。そこで今回はよりユーザー目線でのプロダクトを開発したいと思い、TOKOTOKO=J v2を作成することにしました。**
  
今後の具体的な取り組みとして、
   - 技術的の改善
     - 前回より技術や知識の選択の幅が増えたことにより選択の幅が増えたことにより、よりユーザー目線で、プロダクトに沿った技術選定を行う。　（どの技術がUXが高いまま使われ続けるか。）
     - レンダリング改善、認証機能の改善、画像の処理などUXが低くなっている部分を改修。　（コンポーネントの再利用性、キャッシュ、API通信など）
     - ローンチ後の追加機能なども想定したディレクトリ構成、アーキテクチャにする。
   - 要件定義
...
フロント側と同じ様にディレクトリ構成の軸としたはが**機能ごとの分割**だ。
...
<br />
   - **scssの変数を用いた再利用可能なcss、cssファイルや関数間の改行**
   - **関数やフォルダの一貫した命名(パスカルケース,スネークケース,キャメルケース)**
   - **再利用できるコンポーネントの作成 (アトミックデザインを元にcomponetsフォルダ配下のpartsフォルダやwidgetsフォルダを作成。)**
   - **カスタムフックの作成 (componetsフォルダ配下のhooksフォルダ)**
   - **props以外の型定義の集約**
   - **EsLintを使用したimport orderなどの設定**

 （状態と処理を分ける。Imutabilityの保持。bug修正時に仮説を立てやすくする。コンポーネント影響範囲の特定しやすくする。責務が明らかにする。）
...
+ フロントエンド
  + コンポーネント思考についての理解を深める。大きなプロダクトにも耐えられるディレクトリ構成など。**今回は責務を単一化すること、再利用性を向上させるためにアトミックデザインを採用したが再考する必要がある。**
  + 純粋関数を用いた関数型プログラミング。デバックの可能性も考え、影響範囲を特定しやすくし、保守性を高める。
+ API
  + 今回のAPI設計は不必要なデータの取得やメモリリークの影響、そして多くの画像を含めたfetch処理を行っている。**RESTfulなAPI設計を元に、API設計にも改善が必要。**
  + その対策として、リプレイスの際は【使用経験のある**GraphQLを使用した処理**などの新しい技術を取り入れる】、【画像データを軽量化する処理を行う】必要がある。
  + まずはバックエンドのコードが複雑化を防ぐために、ディレクトリ構成など設計の部分に注力し、土台を固めた後に実装していく必要がある。
    （時間がかかてしまうが、**Swaggerなど最初はモックサーバーで開発を行うことも視野に入れる。**）
