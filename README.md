...
-  要件定義
 ...
 - 足りない機能追加などを行ってしまった。そしてその機能が他のコンポーネントとの依存関係に影響を与えることなどがあり、修正などで時間を費やした。今後の対策として要件の洗い出しの質の向上、そして追加の機能がある際には、与える影響    など単一的な視点になるのではなく多角的な視点を常に持つ。
- バックエンドの修正
 - バックエンドの技術選定は、使用歴の長く【円滑な開発】が行えると言う理由で、Node.jsを使用している。しかし今回のAPIは保守性が低いと感じる。TypeScriptのメリットをバックエンドで最大限活かすことができていない。大量アクセスに優れている点ではNode.jsを選定したことは間違いではないと思う。
