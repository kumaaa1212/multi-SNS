...
- フロントエンドだけではなく、バックエンドやデータベースなど多くの知識選択の幅が増えた。（以前は選択肢が少なく、やりたいけどできないことが多かった。）
- 多くの技術的負債があった。
<br />
以上の3点が挙げられます。
<br />
...
**TOKOTOKO=Jは2023年6月-8月にこれまでの独学の集大成として作成しました。その当時の自分としてかなり完成度が高く、ユーザーの友人やサッカーファンからのポジティブなコメントなどもあり、納得いく形にできたと感じています。**
  <br />
**しかしその後多くのチーム開発などを通して、エンジニアとしての幅が広がりました。そこで今回はよりユーザー目線でのプロダクトを開発したいと思い、TOKOTOKO=J V2を作成することにしました。**
  
今後の具体的な取り組みとして、
   - 技術的な改善
     - 前回より技術や知識の選択の幅が増えたことにより選択の幅が増えたことにより、よりユーザー目線でのプロダクトに沿った技術選定を行う。　（どの技術がUXが高いまま使われ続けるか。）
...
技術選定を行った際の基準は、【円滑な開発】である。
 ...
 - 歴が1年と短く、日々新たなものに触れる状況。その中で「新たに登場したライブラリだから」「人気と言われているから」などの**軽薄な理由での技術選定は開発を円滑に進めることは困難になる**と考えた。
   (本来であればユーザー求めている課題解決に適した最善の技術選定を行いたい。しかし今ある選択肢の中でユーザーに最大限の価値を提供するため、課題を解決するための技術選定を行うことが現段階では重要だと考えた。)
</br>
...
### フロントエンド
 主な使用言語はReact,Next.jsである。処理速度の速さやスタイリング方法の多様性など多くのメリットからReactを採用した。**Next.jsはページ遷移の速さ・pathの指定のし易さなど機能面での優位点**がReact単体よりもあるほか、現在の長期インターンシップ先で使用していることから、**実務に沿ったプロダクト開発を行えると考えた。** グローバルstateを管理するために**Redux**を選んだ。選定の段階でReduxか **【useContext使ったstateの管理】** の2点を検討した。今回のプロダクトの規模ではuseContextで十分であるとも検討したが、今後**新しい機能追加の際やリプレイス**を考えた時に、【状態の管理がし易く】・【データの更新に制約が生まれ、品質を下げにくい】という2点からReduxを選定した。

### バックエンド
バックエンド技術選定を行う際に選択肢として、**Node.jsとRuby(Ruby on Rails)** があった。しかし今回の技術選定の全体の軸として【円滑な開発】としていたため、フロント側でも使用しているTypeScriptを使用できる**Node.js**を選択した。TypeScriptによる静的型付けでの**開発時からの保守性の高さ**、Node.jsの特徴である**大量の同時接続に強い**などもSNSプロダクトでは大きなメリットとなると感じた。そして今回のプロダクトは**フォルダ数、データベース**ともにこれまで自身で開発していたものと比べ、複雑になってしまうことが想定できた。これまでFirebaseなどのNoSQLを使用した開発を行っていたが、今回はプロダクトの大きさやデータベースの複雑性を予想し、**RDBで設計を行うこと**にした。そしてこれまで使用経験が多い**Prisma**を使い、RDBを設計した。
...
アトミックデザインを採用し、コンポーネント毎の責務の単一化、そして再利用性を高めた。
+ pagesにSSRなどのレンダリング時のfetch処理を集約させた。 基本的にpagesに**初回レンダリング時のfetch処理以外**を書かないことで、fetch処理と実際のページを分割している。
+ componets配下に機能ごとのフォルダを設置。(**再利用が可能なもの**は、partsとwidgetsに記述、**実際のページの核**となる部分はtemplateに記述。)
...
# 工夫した点・今回の反省と今後の展開
**自身の大学のサッカー好きや同じチームを応援している友人20人ほどに使っていただき、フィードバックをもらいました。**
# 使っていただいたユーザーの声
- スマホだと使いにくい。
- 情報量が多すぎる。
上記の2点に対するフィードバックが特に多かったです。
<br />
<br />

> スマホだと使いにくい。
 <br />
元々パソコンで使うことを想定したプロダクトでした。なのでマークダウン形式での入力やエラー時の表示などがパソコンを想定したものに寄ってしまっていたと感じている。実際、機能の１つの掲示板機能などはパソコンで投稿することもあるが、スタジアムで試合前などに確認することなども多い。様々な場面でのユーザーを想定したUIに変更しなければいけないと感じた。
<br />
<br />

> 情報量が多すぎる。
 <br />
様々な種類の投稿機能(ブログ形式投稿、短文投稿、画像付き投稿)全てを備えているSNSアプリがないことをきっかけに作成した今回のプロダクトであったが、目的の不明瞭な機能面の充実はUXを下げることにつながると実感した。ユーザーがどのような機能が欲しいのか、どのレベルで十分なのか。類似のサービスを研究し熟考する必要があると感じた。
...
<br />
 ...
 （状態と処理を分ける。Imutabilityの保持。コンポーネント影響範囲の制限。責務が明らかにする。）
<br />
クリーンなコードを意識することで**保守性を高めること**ができる。
バグ修正の際に**修正部分の仮説を立て易くし、迅速な改修につなげることができる。**
## 今回の反省と今後の展開
+ 要件定義
...
+ 設計
  ...
  **上記のように、開発前の要件定義や設計などの開発作業前の品質向上が課題であると考える。 
  　</br>
  (様々なユーザーを想定する→求めていることを細分化する→取捨選択する)
  </br>
  ただし、要件定義や設計を実施する機会は頻繁に訪れるわけではない。普段から開発を行う際、現在の仕様に至った経緯や背景を確認、意識しながら開発を行う必要だ。**
...
+ 機能面
  + ユーザー目線での改善が必要。色々なSNSをまとめたのでプロダクト全体の機能は充実している。しかし**機能面の充実がUXの向上に繋がる訳ではない。**
  + 想定するユーザーが比較的高めであること（20-50代）も含め、**プロダクト内での機能の明確な分割（このページはこの機能とわかるようにする）、機能の取捨選択**などが必要になる。
  + 今回アルバム機能の入力方法は【markdown形式での記入】だ。しかしスマートフォンでのmarkdown記入はかなり困難である。ただレスポンシブ対応（画面を小さくしてもレイアウト崩れない）をするだけではなく、スマートフォンでの利用を想定したUI改善が必要になる。
+ API
  + 今回のfetch処理は多くの画像を含めたfetch処理などにより、不必要なデータの取得やメモリリークの影響を大きく受けている。**RESTfulなAPI設計を元に、API設計にも改善が必要。**
  + その対策として、リプレイスの際はフロントエンド側でのコードの再考、また【使用経験のある**GraphQLを使用した処理**などの新しい技術を取り入れる】、【画像データを軽量化する処理を行う】必要がある。
  + まずはバックエンドの複雑化を防ぐために、ディレクトリ構成など設計の部分に注力し、土台を固めた後に実装していく必要がある。
    （時間がかかてしまうが、**Swaggerなど最初はモックサーバーで開発を行うことも視野に入れる。** フロント開発後→バックエンド開発）
